...
public class Player_OLD {
	...
	public Player_OLD(int aWidthTiledMap, int aHeightTiledMap, AssetManager assetManager) {
	...
	public void updatePlayerPosition(float delta, List<Rectangle> tiles, ArrayList<Vehicle> vehiclesList) {
		...
		float newPositionX = playerAnimation.getX()+velocity.x*delta;
		float newPositionY = playerAnimation.getY()+velocity.y*delta;
		...
		if(!checkTilesColision(newPositionX, newPositionY, tiles)){
			playerAnimation.setX(newPositionX);
			playerAnimation.setY(newPositionY);
		}

		//regras de parada de movimento
		if(movendoX1 && playerAnimation.getX() + playerAnimation.getWidth()/2 >= pontoX) {
			movendoX1 = false;
			velocity.x = 0;
		}

		if(movendoY1 && playerAnimation.getY()+playerAnimation.getHeight()/2 >= pontoY) {
			movendoY1 = false;
			velocity.y = 0;
		}

		if(movendoX2 && playerAnimation.getX()+playerAnimation.getWidth()/2 <= pontoX) {
			movendoX2 = false;
			velocity.x = 0;
		}

		if(movendoY2 && playerAnimation.getY()+playerAnimation.getHeight()/2 <= pontoY) {
			movendoY2 = false;
			velocity.y = 0;
		}
	}
	...
	private boolean checkVehiclesColision(ArrayList<Vehicle> vehiclesList){
		...
		//recebe a posicao atual do player para checar colisão com veiculos
		Rectangle playerPosition = new Rectangle(playerAnimation.getX(), playerAnimation.getY(), Constantes.WIDTH_PLAYER, Constantes.HEIGHT_PLAYER);
	...
	public void movimentar(int screenX, int screenY, OrthographicCamera orthographicCamera) {
		...
		if(pontoX > playerAnimation.getX()){
			velocity.x = speed;
			movendoX1 = true;
		...
		if(pontoY > playerAnimation.getY()){
			velocity.y = speed;
			movendoY1 = true;
		}
		if(pontoY < playerAnimation.getY()){
	...
	public void inicializar(Vector2 vector2) {
		playerAnimation.setPosition(vector2.x, vector2.y);
		velocity.x = 0;
		velocity.y = 0;
		movendoX1 = false;
		movendoX2 = false;
		...
		playerLife.inicializar();
		playerAnimation.inicializar();

	}

	public void dispose() {
		playerAnimation.getTexture().dispose();
	}
	...
	public void draw(Batch batch, float delta){
		...
		if(demage){
			t += delta;
			playerAnimation.setAlpha(Util.getRandomPosition(3, 8)/10);
